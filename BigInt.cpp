...
BigInt BigInt::add_diff_signs(const BigInt& big, const BigInt& small) {
	...
	for (int ind = result_digits.size() - 1; ind >= 0; --ind) {
...
BigInt BigInt::karatsuba_mult(const BigInt& x, const BigInt& y) {
	...
	if (x_n == y_n) {
		...
		if (biggest->digits.size() < smallest->digits.size()) { // x and y have a different number of digits, need to know which number has least digits
			std::swap(biggest, smallest);
		}
		BigInt padded(*smallest);
		padded.digits.insert(padded.digits.end(), biggest->digits.size() - smallest->digits.size(), 0); // pad the smaller number with leading 0s
...
//	if (x_n < 75 || y_n < 75) {
//		return long_mult(x, y); // karatsuba overhead not worth it, just do grade-school multiplication
//	}
//
//	const size_t n = std::max(x_n, y_n);
//	const size_t m = (n / 2) + (n % 2); // n/2 if n is even, (n+1)/2 if n is odd
//
//	BigInt a, b, c, d;
//	a.digits.reserve(m + 1); b.digits.reserve(m + 1); c.digits.reserve(m + 1); d.digits.reserve(m + 1);
//	size_t x_cnt = 0;
//	size_t y_cnt = 0;
//
//	while (x_cnt < m && x_cnt < x_n) { // initialize b to m least significant digits of x
//		if (x_cnt == 0) {
//			b.digits.pop_back();
//		}
//		b.digits.push_back(x.digits[x_cnt++]);
//	}
//	while (x_cnt < x_n) { // initialize a to remaining most significant digits of x
//		if (x_cnt == m) {
//			a.digits.pop_back();
//		}
//		a.digits.push_back(x.digits[x_cnt++]);
//	}
//	while (y_cnt < m && y_cnt < y_n) { // initialize c to m least significant digits of y
//		if (y_cnt == 0) {
//			d.digits.pop_back();
//		}
//		d.digits.push_back(y.digits[y_cnt++]);
//	}
//	while (y_cnt < y_n) { // initialize d to remaining most significant digits of y
//		if (y_cnt == m) {
//			c.digits.pop_back();
//		}
//		c.digits.push_back(y.digits[y_cnt++]);
//	}
//
//	// if x = a*BASE^m + b and y=c*BASE^m + d
//	// then xy = ac*BASE^(2m) + (ad+bc)*BASE^m + bd
//	BigInt ac = karatsuba_mult(a, c);
//	BigInt bd = karatsuba_mult(b, d);
//	BigInt ad_plus_bc = karatsuba_mult(a + b, c + d) - ac - bd; // (a+b)(c+d)=ac+ad+bc+bd ==> ac+ad+bc+bd-ac-bd=ad+bc
//
//	ac.digits.insert(ac.digits.begin(), 2*m, 0); // multiply ac by BASE^(2m)
//	ad_plus_bc.digits.insert(ad_plus_bc.digits.begin(), m, 0); // multiply (ad+bc) by BASE^m
//
//	return ac + ad_plus_bc + bd;
//}
